        ...
        <div className="flex flex-wrap gap-2">
          {STATION_LINKS.map(([label, href]) => (
            <a key={href} href={href} target="_blank"
              className="rounded-lg bg-zinc-100 px-3 py-1.5 text-sm hover:bg-zinc-200">{label} ↗</a>
          ))}
        </div>
      </Card>
...
function TeamRow({ team, onChange }: { team: Snapshot["teams"][number]; onChange: () => void }) {
  ...
  return (
    <div className="flex flex-wrap items-end gap-2 border-b border-zinc-100 pb-2">
      <input value={name} onChange={(e) => setName(e.target.value)} className="w-28 rounded-lg border border-zinc-300 px-2 py-1.5 text-sm" />
      <label className="text-xs text-zinc-400">光幣<input type="number" value={coins} onChange={(e) => setCoins(Number(e.target.value) || 0)} className="ml-1 w-24 rounded-lg border border-zinc-300 px-2 py-1.5 text-sm" /></label>
      <label className="text-xs text-zinc-400">點數<input type="number" value={points} onChange={(e) => setPoints(Number(e.target.value) || 0)} className="ml-1 w-20 rounded-lg border border-zinc-300 px-2 py-1.5 text-sm" /></label>
      <span className="text-xs text-zinc-400">淨資產 {team.netWorth}</span>
      <ActionButton label="儲存" className="bg-zinc-900 text-white"
      ...
      <div className="mb-3 flex gap-2">
        {REGIONS.map((r) => (
          <button key={r.code} onClick={() => setRegion(r.code)}
            className={`rounded-lg px-3 py-1.5 text-sm ${region === r.code ? "bg-zinc-900 text-white" : "bg-zinc-100"}`}>{r.name}</button>
        ))}
      </div>
...
function PropRow({ prop, teams, onChange }: { prop: Snapshot["properties"][number]; teams: Snapshot["teams"]; onChange: () => void }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 text-sm">
      ...
      <span className="w-16 text-xs text-zinc-400">{prop.type}</span>
      <span className="w-16 tabular-nums text-zinc-500">現價{prop.currentValue}</span>
      <select value={owner} onChange={(e) => setOwner(e.target.value)} className="rounded-lg border border-zinc-300 px-2 py-1 text-sm">
        <option value="">未售出</option>
        {teams.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
      </select>
      <select value={level} onChange={(e) => setLevel(Number(e.target.value))} className="rounded-lg border border-zinc-300 px-2 py-1 text-sm">
        {[0, 1, 2, 3].map((l) => <option key={l} value={l}>{l}級</option>)}
      </select>
      <ActionButton label="儲存" className="bg-zinc-200 text-zinc-700"
...
function CardRow({ card, onChange }: { card: { type: string; cost: number; remaining: number }; onChange: () => void }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 text-sm">
      ...
      <label className="text-xs text-zinc-400">點數<input type="number" value={cost} onChange={(e) => setCost(Number(e.target.value) || 0)} className="ml-1 w-20 rounded-lg border border-zinc-300 px-2 py-1 text-sm" /></label>
      <label className="text-xs text-zinc-400">庫存<input type="number" value={remaining} onChange={(e) => setRemaining(Number(e.target.value) || 0)} className="ml-1 w-20 rounded-lg border border-zinc-300 px-2 py-1 text-sm" /></label>
      <ActionButton label="儲存" className="bg-zinc-200 text-zinc-700"
        ...
        {data?.map((r) => (
          <li key={r.id} className="flex items-center justify-between py-1.5">
            <span className={r.reversed ? "text-zinc-400 line-through" : ""}>
              ...
              <span className="ml-1 text-xs text-zinc-400">{r.byToken}</span>
            </span>
