...
本系統引入了四大區域獨佔被動效果與房屋被動營收機制，並配合市場卡與不動產買賣回收，提升不動產策略深度。

### 四大區域獨佔被動效果

各區域最多三棟全部獨佔時，該隊伍自動獲得專屬效果：

| 區域 | 獨佔效果 | 說明 |
| --- | --- | --- |
| AURORA（極光） | 光幣收益 ×1.5 | 過路費、房屋被動收益、動產收益、流動關發獎、好運卡獎勵應用此倍率（不含大樂透與輪盤）。加成由銀行補發給收款方，守恆不破。 |
| SPECTRA（光譜） | 每回合 +10 卡牌點數 | 地圖關主觸發每輪結算時，該隊自動獲得 10 點卡牌點數。 |
| EMBER（炎焰） | 全場升級加速 | 該隊購買新地產時直接升至 1 級，升級操作每次 +2 級（可從 1 跳至 3），只收一次升級費用（仍按現值計費；該隊對其他隊過路費計算時仍按實際級別）。 |
| HAVEN（避風港） | 實時升值 | 該隊全部不動產隨真實時間線性上升，每秒 0.001%。升值在四時機鎖定進 monopolyBonusMult：發動收益、隊伍更換時 reconcile、比賽結算前 flush、賣地時取當前值。每次鎖定後 since 重設，重複 flush 不重複計。 |

### 房屋被動營收

升級至 1 / 2 / 3 級的房屋在每輪結算時自動產生光幣收益，金額 = **現值 × 級別費率**（預設 3% / 5% / 8%），四捨五入至個位。此收益受 AURORA 獨佔倍率影響。

### 市場卡與倍率疊加

四張市場卡提供區域或單棟倍率調整，永久疊乘無遞減上限：

| 卡名 | 效果 | 次數 | 可撤銷 |
| --- | --- | --- | --- |
| 紅卡（區域漲） | 整區所有不動產現值 ×1.2 | 1 次 | 是 |
| 黑卡（區域跌） | 整區所有不動產現值 ×0.8 | 1 次 | 是 |
| 鬧鬼卡（單棟跌） | 指定單棟現值 ×0.7 | 1 次 | 是 |
| 土地公卡（單棟漲） | 指定單棟現值 ×1.3 | 1 次 | 是 |

卡牌可打自己持有的不動產，無主地亦可打。倍率直接相乘，套用於現價計算與過路費、房屋收益等衍生值。

### 賣不動產回收

隊伍可於交易所將不動產賣回銀行，收回金額 = **investedValue**（含升級本金與所有事件/卡牌倍率，取整到 10）。該筆回收金額包含 AURORA 倍率（若團隊獨佔極光區）。賣地後倍率狀態保留，不重置；HAVEN 升值在賣地時鎖進 monopolyBonusMult。賣地後該棟由交易所重新控管，待其他隊伍購買。

### Admin 可調參數

Admin 於「不動產進階參數」面板可實時調整所有數值，立即生效：

- AURORA、SPECTRA、EMBER、HAVEN 各效果的倍率與基數
- 房屋被動營收費率（1/2/3 級）
- 市場卡倍率（紅/黑/鬧鬼/土地公）

參數變更無需重啟，當前進行中的盤面即時套用。
